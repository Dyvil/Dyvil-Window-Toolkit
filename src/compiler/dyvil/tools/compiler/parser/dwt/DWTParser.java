...
public class DWTParser extends Parser implements IValued
{
	...
	public void parse(IParserManager pm, IToken token)
	{
		...
		if (this.mode == NAME)
		{
			...
			pm.report(new SyntaxError(token, "Invalid DWT File - Name expected"));
		}
		if (this.mode == BODY)
		{
			...
			pm.report(new SyntaxError(token, "Invalid Body - '{' expected"));
		}
		...
		if (this.mode == PROPERTY_NAME)
		{
			...
			this.mode = PROPERTY_NAME;
			pm.report(new SyntaxError(token, "Invalid Property - Name expected"));
		}
		if (this.mode == EQUALS)
		{
			...
			this.mode = PROPERTY_NAME;
			pm.report(new SyntaxError(token, "Invalid Property - '=' expected"));
		}
